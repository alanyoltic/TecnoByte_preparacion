    ...
    <div class="relative z-10 w-full px-4 sm:px-6 lg:px-8 pt-6 pb-10 space-y-6">
        ...
        @if($vista === 'lista')
            <x-topbar
                ...
                chip="Ventas · Inventario"
                description="Órdenes de compra de productos y consumibles para el área de ventas."
            >
                <x-slot name="right">
                    <div class="flex items-center gap-2">
                        @can('ventas.compras.gestionar')
        ...
        @elseif($vista === 'nueva')
            <x-topbar
                ...
                chip="Ventas · Compras"
                description="Registra una orden de compra a proveedor. Elige entre productos y/o consumibles."
            >
                <x-slot name="right">
        ...
        @if($vista === 'lista')
            ...
            {{-- Métricas --}}
            @php
                $totalCompras     = \App\Models\CompraInventario::deArea('VENTAS')->count();
                $totalProductos   = \App\Models\CompraInventarioItem::whereHas('compra', fn($q) => $q->deArea('VENTAS'))->whereNotNull('producto_id')->sum('cantidad');
                $totalConsumibles = \App\Models\CompraInventarioItem::whereHas('compra', fn($q) => $q->deArea('VENTAS'))->whereNotNull('consumible_id')->sum('cantidad');
                $totalInvertido   = \App\Models\CompraInventario::deArea('VENTAS')->whereNotNull('total_estimado')->sum('total_estimado');
            @endphp
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div class="rounded-2xl bg-emerald-50/90 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-500/70
                    ...
                    <p class="text-xs font-semibold text-emerald-700 dark:text-emerald-200 uppercase tracking-wide">Productos</p>
                    <p class="mt-2 text-2xl font-bold text-emerald-800 dark:text-emerald-100">{{ (int)$totalProductos }}</p>
                </div>
                <div class="rounded-2xl bg-purple-50/90 dark:bg-purple-950/40 border border-purple-200/80 dark:border-purple-500/70
                            backdrop-blur-xl px-4 py-3 shadow-md transition-all duration-300 hover:-translate-y-1
                            hover:shadow-lg hover:shadow-purple-500/40">
                    <p class="text-xs font-semibold text-purple-700 dark:text-purple-200 uppercase tracking-wide">Consumibles</p>
                    <p class="mt-2 text-2xl font-bold text-purple-800 dark:text-purple-100">{{ (int)$totalConsumibles }}</p>
                </div>
                    ...
                    <table class="min-w-full text-sm text-left">
                        <thead class="bg-slate-100 dark:bg-slate-950/90 border-b border-slate-200 dark:border-slate-800/80">
                            <tr>
                                ...
                                <th class="px-5 py-3 font-semibold text-slate-700 dark:text-slate-300 text-center">Items</th>
                            ...
                            @forelse($this->compras as $compra)
                                <tr class="border-b border-slate-200 dark:border-slate-800/80
                                    ...
                                    <td class="px-5 py-3">
                                        ...
                                        @if($compra->loteDestino)
                                            <p class="text-xs text-slate-400 mt-0.5">📦 {{ $compra->loteDestino->nombre }}</p>
                                        @endif
                                    </td>
            ...
            @if($verCompraId && $this->compraDetalle)
                ...
                <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div class="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl
                        ...
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-xs text-slate-400 mt-0.5">
                                    ...
                                    @if($d->loteDestino) · 📦 Lote: {{ $d->loteDestino->nombre }} @endif
                        ...
                        @if($d->items->whereNotNull('producto_id')->count() > 0)
                            <div>
                                <p class="text-xs uppercase tracking-wider font-semibold text-slate-500 dark:text-slate-400 mb-2">📦 Productos</p>
                                <div class="rounded-xl border border-slate-200/80 dark:border-slate-800 overflow-hidden">
                                    @foreach($d->items->whereNotNull('producto_id') as $item)
                                        <div class="flex justify-between items-center px-4 py-2.5 text-sm
                                            ...
                                            <div>
                                                <span class="font-medium text-slate-800 dark:text-slate-200">{{ $item->producto?->nombre ?? '—' }}</span>
                                                <span class="text-xs text-slate-400 ml-1.5">[{{ $item->producto?->categoria }}]</span>
                                            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="rounded-2xl bg-white/80 dark:bg-slate-950/70 border border-slate-200/80 dark:border-white/10
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach(['productos' => '📦 Productos', 'consumibles' => '🧽 Consumibles', 'todos' => '📦 Todo'] as $val => $label)
                ...
                {{-- ── PASO 3: Tabla de PRODUCTOS ─────────────────────── --}}
                @if(in_array($tipoCompra, ['productos', 'todos']))
                <div class="rounded-2xl bg-white/80 dark:bg-slate-950/70 border border-slate-200/80 dark:border-white/10
                            backdrop-blur-xl shadow-md overflow-hidden">
                    <div class="px-5 py-4 border-b border-slate-200/60 dark:border-slate-800/80 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <span class="text-lg">📦</span>
                            <h3 class="text-sm font-semibold text-slate-800 dark:text-slate-100">Productos del catálogo</h3>
                        </div>
                        <button wire:click="agregarItemProducto"
                    ...
                    <div class="px-4 py-3 space-y-2">
                        @foreach($itemsProductos as $idx => $item)
                            <div class="flex flex-wrap gap-2 items-start bg-slate-50/80 dark:bg-slate-900/40
                                ...
                                <div class="flex-1 min-w-[200px]">
                                    <select wire:model="itemsProductos.{{ $idx }}.producto_id"
                                        ...
                                        @foreach($this->catalogoProductos as $prod)
                                            <option value="{{ $prod->id }}">
                                                [{{ $prod->categoria }}] {{ $prod->nombre }}
                                ...
                                {{-- Cantidad --}}
                                <div class="w-20">
                                    <input type="number" min="1" wire:model="itemsProductos.{{ $idx }}.cantidad"
                                        placeholder="Cant."
                                        class="w-full rounded-xl bg-white dark:bg-slate-900 border border-slate-300/70 dark:border-slate-700
                                ...
                                {{-- Precio unitario --}}
                                <div class="w-28">
                                    <input type="number" step="0.01" min="0" wire:model="itemsProductos.{{ $idx }}.precio_unitario"
                                        placeholder="$ Precio"
                                        class="w-full rounded-xl bg-white dark:bg-slate-900 border border-slate-300/70 dark:border-slate-700
                                               text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 px-3 py-2">
                                </div>
                                ...
                                <div class="flex-1 min-w-[100px]">
                                    <input type="text" wire:model="itemsProductos.{{ $idx }}.notas"
                                ...
                                <div class="flex items-center gap-1.5 pt-0.5">
                                    @if(count($itemsProductos) > 1)
                                        <button wire:click="removerItemProducto({{ $idx }})"
                    ...
                    <div class="flex gap-3">
                        ...
                        <button wire:click="guardarCompra" wire:loading.attr="disabled"
                            class="px-6 py-2.5 rounded-xl text-sm font-semibold text-white
                                   ...
                                   hover:shadow-emerald-500/60 hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-70">
                            Registrar compra
                        </button>
                    </div>
                </div>
...
@if($modalProveedor)
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div class="w-full max-w-md rounded-2xl bg-white dark:bg-slate-900
            ...
            <div class="space-y-3">
                <div>
                    <label class="text-xs font-semibold text-slate-600 dark:text-slate-400">Nombre de la empresa *</label>
                    <input type="text" wire:model="proveedorNombre" placeholder="Ej: CompraTech S.A."
                ...
                <div>
                    ...
                    <input type="text" wire:model="proveedorAbreviacion" placeholder="Ej: CTECH"
                        class="mt-1 w-full rounded-xl bg-white dark:bg-slate-800 border border-slate-300/70 dark:border-slate-700
                               text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 px-3 py-2.5 uppercase">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="email" wire:model="proveedorEmail" placeholder="opcional"
                            class="mt-1 w-full rounded-xl bg-white dark:bg-slate-800 border border-slate-300/70 dark:border-slate-700
                                   text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 px-3 py-2.5">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-slate-600 dark:text-slate-400">Teléfono</label>
                        <input type="text" wire:model="proveedorTelefono" placeholder="opcional"
                            class="mt-1 w-full rounded-xl bg-white dark:bg-slate-800 border border-slate-300/70 dark:border-slate-700
                                   text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 px-3 py-2.5">
                    </div>
                </div>
