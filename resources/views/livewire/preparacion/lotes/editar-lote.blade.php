...
<x-tb-background>
    ...
    {{-- Línea --}}
    <div class="border-t border-slate-200 dark:border-slate-800/70 pt-1 mt-6 mb-4"></div>

    {{-- ===== CARGADORES ===== --}}
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <p class="text-[0.75rem] font-semibold tracking-wide text-slate-500 dark:text-slate-400 uppercase">
                Cargadores en este lote
            </p>
        </div>

        @if(!empty($cargadoresExistentes))
            <div class="space-y-2">
                <p class="text-xs font-semibold text-slate-500">Cargadores ya registrados:</p>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    @foreach($cargadoresExistentes as $ce)
                        <div class="flex items-center justify-between px-3 py-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg">
                            <div class="flex flex-col">
                                <span class="text-xs font-bold text-slate-700 dark:text-slate-200">{{ $ce['serie'] ?? 'Sin serie' }}</span>
                                <span class="text-[0.65rem] text-slate-500">{{ $ce['marca'] ?? 'Genérico' }} | {{ $ce['voltaje'] }}V {{ $ce['amperaje'] }}A ({{ $ce['punta'] }})</span>
                            </div>
                            <button type="button" wire:click="removeCargadorExistente({{ $ce['id'] }})" wire:confirm="¿Seguro que deseas eliminar este cargador del sistema?" class="text-red-400 hover:text-red-600">
                                ✕
                            </button>
                        </div>
                    @endforeach
