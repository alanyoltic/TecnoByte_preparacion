...
class BackfillEquipoGpus extends Command
{
    ...
    public function handle(): int
    {
        ...
        $normText = function ($v): ?string {
            ...
            if (in_array($up, ['NO', 'N/A', 'NA', 'NULL', 'SIN', '0'], true)) {
                return null;
            }
        ...
        $inferMarca = function (?string $modelo): ?string {
            ...
            $u = strtoupper($modelo);
            if (str_contains($u, 'NVIDIA') || str_contains($u, 'GEFORCE') || str_contains($u, 'RTX') || str_contains($u, 'GTX') || str_contains($u, 'QUADRO')) {
                return 'NVIDIA';
            }
            if (str_contains($u, 'AMD') || str_contains($u, 'RADEON') || str_contains($u, 'VEGA')) {
                return 'AMD';
            }
            if (str_contains($u, 'INTEL') || str_contains($u, 'IRIS') || str_contains($u, 'UHD')) {
                return 'INTEL';
            }
